	...
	class lumise_connector {
        ...
        public function filter_product($data) {
			...
			*/	
			...
			$cms_product = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}posts` WHERE `id`=".$data['product']);
			...
			if (
				...
				if (isset($stages->stages)) {
					foreach ($stages->stages as $s => $d) {
						if (isset($cms_template[$s]))
							$stages->stages->{$s}->template = $cms_template[$s];
						else unset($stages->stages->{$s}->template);
					}
				} else {
					foreach ($stages as $s => $d) {
						if (isset($cms_template[$s]))
							$stages->{$s}->template = $cms_template[$s];
						else unset($stages->{$s}->template);
					}
				}
				
				$data['templates'] = $cms_template;
				$data['stages'] = $lumise->lib->enjson($stages);
		...
		public function query_products($query, $args) {
			...
			return array(
				...
				(!empty($args['category']) ?
					...
					"WHERE p.active = 1 AND wpp.post_status = 'publish' AND cref.category_id = ".$args['category'].$search
					:
					"WHERE p.active = 1 AND wpp.post_status = 'publish'".$search
				),
				"GROUP BY p.id",
							...
							var cart_data =  JSON.parse(localStorage.getItem('LUMISE-CART-DATA') || '{}'),
								...
								addcart = function(i) {
									...
									$.get('<?php echo $lumise->cfg->upload_url; ?>designs/'+orders[i].design+'.lumi', function(res) {
										...
										data.product_data.stages = lumise.func.dejson(data.product_data.stages);
										
										cart_data[res.id] = lumise.func.enjson(data);
										...
										if (orders[i+1] !== undefined) {
											...
											lumise.func.notice('<?php echo $lumise->lang('Your order has been successfully replicated'); ?>', 'success', 5000);
											lumise.func.set_url('reorder', null);
