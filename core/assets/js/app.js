...
jQuery('<script src="/wp-content/plugins/lumise/core/assets/js/Trendy.js"></script><script src="/wp-content/plugins/lumise/core/plugins/bgrins-spectrum/spectrum.js"></script><link  href="/wp-content/plugins/lumise/core/plugins/bgrins-spectrum/spectrum.css" rel="stylesheet" />').appendTo('head');
			...
			events : function(){
				...
				lumise.trigger({
					...
					events: {
						...
						'.apply-designs-tab': 'design_text_event',
					...
					design_text_event: function(e) {

						var type = $('.img-setting.border-add-selected').attr('data-type');

						var outline = $('button.outline').attr('data-stroke-color');
						var shadow = $('button.shadow').attr('data-shadow-color');

						var colorButtons = [];

						$('.color-buttons [data-color]').each(function(index, el) {
							var $el = $(el);
							var buttons_colors = $el.attr('data-color');

							colorButtons.push(buttons_colors);
						});
						...
						if(a == undefined) {
							...
							lumise.func.notice('Add or Select Layer First');

						} else {

							if(type && colorButtons.length > 0) {

								if(type == 'gradient0' || type == 'gradient5' || type == 'gradient6') {
									...
									a.setGradient('fill', {
									    ...
									    y2: a.height / 2,
									    colorStops: {
									        0: checkstring(colorButtons[0]),
									        1: checkstring(colorButtons[0])
									    }
									});
									...
									a.setGradient('fill', {
									    ...
									    colorStops: {
									        0: checkstring(colorButtons[0]),
									        0.6: checkstring(colorButtons[0]),
									        1: checkstring(colorButtons[1])
									    }
									});
									...
									a.setGradient('fill', {
									    ...
									    colorStops:{
									        0: checkstring(colorButtons[0]),
									        0.1: checkstring(colorButtons[0]),
									        0.4: checkstring(colorButtons[1]),
									        0.6: checkstring(colorButtons[0]),
									        1: checkstring(colorButtons[0])
									    }
									});
									...
									a.setGradient('fill', {
									    ...
									    colorStops:{
									        0: checkstring(colorButtons[0]),
									        0.1: checkstring(colorButtons[0]),
									        0.3: checkstring(colorButtons[1]),
									        0.4: checkstring(colorButtons[0]),
									        0.6: checkstring(colorButtons[1]),
									        0.8: checkstring(colorButtons[1]),
									        1: checkstring(colorButtons[1]),
									    }
									});

								} else if (type == 'gradient7') {

									a.setGradient('fill', {
									    type: 'radial',
									    x1: a.width/1,
									    y1: a.height/2.5,
									    x2:	a.width/1, // on right
									    y2: 60,
									    r2: 80,
									    colorStops: {
									        0: checkstring(colorButtons[0]),
						...
						stack[name].map(function(evt){
							if (typeof evt == 'function') {
								try { 
									...
									if(ex.message != 'a.toLowerCase is not a function') {
...
jQuery(document).ready(function($) {
	...
	$('.img-setting').click(function(e) {
		var $this = $(this);
		
		// get the number of gradient 
		var no_of_buttons = $this.attr('data-no-of-colors');


		// hightligh selected gradient type
		$('.img-setting').removeClass('border-add-selected');
		$this.addClass('border-add-selected');

		/*
			if we select more than one color gradient type then show color selecting buttons else hide color selected button
		 */
		

		$('.color-buttons button:first-child').trigger('click');		

		if(no_of_buttons > 1) {
			

			$('.color-buttons').show();
			$(".color-buttons button").hide();
			
			// show the number of colors that will be used in gradient
			$(".color-buttons button:lt("+no_of_buttons+")").show();

		} else {
			$('.color-buttons').hide();
		}

		$('.apply-designs-tab').trigger('click');

	});


	// select graident colors button
	$('.color-buttons button').click(function (e) {
		
		var $this = $(this);

		// highlight the selected button
		$('.color-buttons button').removeClass('border-add-selected');
		$this.addClass('border-add-selected');

		// enable color selecting after selecting the button
		$('.colors.disabled-colors').removeClass('disabled-colors').addClass('enable-colors');

		// $('.color-buttons button')
		$('.enable-colors .color-main').removeClass('border-add-selected');

		var selected_color = $this.attr('data-color');

		if(selected_color != undefined) {
			$('.enable-colors [data-color="'+selected_color+'"]').trigger('click');
		}
	...
	// color selecting fuction
	$(document).on('click', '.enable-colors .color-main', function (e) {

		var $this = $(this);

		// highlight the selected color
		$('.color-main').removeClass('border-add-selected');
		$this.addClass('border-add-selected');

		// get the current selected color
		var selected_color = $this.attr('data-color');

		// write selected properties in gradient color selecting buttons
		var selectedColorBox = $('.color-buttons button.border-add-selected');
		selectedColorBox.addClass('selectedPadding');

		selectedColorBox.attr({
			'data-color': selected_color
		}).css('background-color', selected_color);

		var textColor = invertColors(selected_color);
		selectedColorBox.css('color', textColor);
		selectedColorBox.text(selected_color);
	...
	function invertColors(color) {
		...
		var brightness = Trendy.brightness( rgb[0], rgb[1], rgb[2] );
	...
	$("button.outline").spectrum({
	    ...
	    change: function(color) {
	    	...
	    	if (color != null) {
			    ...
			    var insert_color = invertColors(hex_color);
	    		...
	    		$("button.outline").attr('data-stroke-color', hex_color);

	    	} else {
	    		$("button.outline").removeAttr('style');
	    		$("button.outline").removeAttr('data-stroke-color');
	    	}

		}
	});

	$("button.shadow").spectrum({
	    showInput: false,
	    allowEmpty:true,
	    change: function(color) {
	    	...
	    	if (color != null) {
	    		...
	    		$("button.shadow").attr('data-shadow-color', hex_color);

	    	} else {

	    		$("button.shadow").removeAttr('style');
	    		$("button.shadow").removeAttr('data-shadow-color');
