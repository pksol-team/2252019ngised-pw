...
class lumise_admin extends lumise_lib{
	...
	public function process_data($args, $name) {
		...
		$args = $lumise->apply_filters('fields-'.$name, $args);
		...
		if (isset($_POST['lumise-section'])) {
			...
			if ($section == 'font') {
				...
				do {
					...
					$fquery .= " WHERE `name` = '".$lumise->lib->sql_esc($data['name'])."'";
					if (!empty($data_id))
	...
	public function process_callback($id = 0, $cb = '') {
		...
		switch ($cb) {
			case 'edit-cms-product':
				$data = $lumise->db->rawQuery("SELECT `name`,`stages`,`attributes` FROM `{$lumise->db->prefix}products` WHERE `id`=$id");
	...
	public function breadcrumb($lumise_page, $type = null) {
		...
		$arr = $lumise->apply_filters('admin_breadcrumb', array(
			...
			'addons' => array(
				...
				'link'   => $lumise_router->getURI().'lumise-page=addons',
				'child' => array(
					'explore-addons' => array(
						'type'   => '',
						'title'  => $lumise->lang('Explore'),
						'link'   => $lumise_router->getURI().'lumise-page=explore-addons',
					),
					'addons' => array(
						'type'   => '',
